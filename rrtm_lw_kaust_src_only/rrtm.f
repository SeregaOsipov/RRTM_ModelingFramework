...
C      character(len=11) :: inputAerFileNamePrefix = "IN_AER_RRTM"
...
C      character(len=211) :: inputAerFileName = ""
...
c     construct correct new file names for input and output
      ...
      outputTapeFN = outputTapeFileNamePrefix//inputArg
      outputCldFN = outputCldFileNamePrefix//inputArg

      inputFileName = inputFileNamePrefix//inputArg
C      inputAerFileName = inputAerFileNamePrefix//inputArg
...
C ***    Input atmospheric profile from INPUT_RRTM.
         CALL READPROF(inputCldFileName, outputCldFN, outputTapeFN)
...
C************************  SUBROUTINE READPROF  *****************************C
      ...
      SUBROUTINE READPROF(inputCldFN, outputCldFN, outputTapeFN)
...
c     clouds input and output file names
      character(len=211) :: inputCldFN
      character(len=212) :: outputCldFN
C	  TAPE output file name      
      character(len=205) :: outputTapeFN
...
C     If clouds are present, read in appropriate input file, IN_CLD_RRTM.
      IF (ICLD .GE. 1) CALL READCLD(inputCldFN, outputCldFN)
...
C************************  SUBROUTINE READCLD  *****************************C
      ...
      SUBROUTINE READCLD(inputCldFN, outputCldFN)
...
C Open OUT_CLD_RRTM to output the cloud optical properties
      ICD = 25
      OPEN (ICD,FILE=outputCldFN,FORM='FORMATTED')

      WRITE(ICD,8894) NUMANGS
      WRITE(ICD,8895) NSTR

C Open Cloud Input File
      OPEN(IRDCLD,FILE=inputCldFN,FORM='FORMATTED')
