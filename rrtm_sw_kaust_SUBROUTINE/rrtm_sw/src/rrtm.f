C
C modified and converted to subroutine by Sergey Osipov (Serega.Osipov@gmail.com)
C
...
*                                                                          *
****************************************************************************
       subroutine RRTM_SW(IAER, IATM, ISCAT, ISTRM, IOUT, ICLD, IDELM,
...
C	   H1F,H2F are record 3.2 HBOUND and HTOA
C		IBMAX_B should always be negative to set up layer boundaries in pressure


c	  use control_variables, only : testVar

C      IMPLICIT DOUBLE PRECISION (V)
c      implicit none
      INCLUDE 	'param.f'
C	  use rrtm_sw_parameters, only : mxlay
...
C        level and the heating rate for each layer
     ...
     *             HVRKG28,HVRKG29
      ...
      real*8 :: SEMISS(NBANDS)
      real*8 :: SOLVAR(NBANDS)
      real*8 PBND(MXLAY)
	  real*8 ZMDL(MXLAY)
	  real*8 PM(MXLAY)
	  real*8 TM(MXLAY)
	  ...
	  integer :: ISTRM, ICLD, IDELM, ICOS, JULDAT
	  real :: SZA
	  integer :: ISOLVAR
	  integer :: NSTR, ISTART, IEND, isccos
...
C		C  Initialize molecular amount and cross section arrays to zero here.
...
C	  //TODO: add clouds support
C     IF (ICLD .EQ. 1) 
c			CALL READCLD(IAER,NSTR,IOUT,ISTART,IEND,ICLD,idelm, isccos)
c	  endif

C     If aerosols are present, read in appropriate input file, IN_AER_RRTM.
C	  //TODO: add aerosol support
C      IF (IAER.EQ.10) CALL READAER(IAER, NSTR, IOUT, ISTART, IEND, ICLD,
c     &                   idelm, isccos)
...
C         ENDIF
      ELSE
         ...
         CALL RRTATM(IBMAX_B, H1F, H2F, CO2MX, PBND, ZMDL, PM, TM)
         IF (IXSECT .EQ. 1) THEN
            DO 3300 MX = 1, NXMOL0
...
C        by interpolating data from stored reference atmospheres. 
         ICLDATM = 0
c		 //TODO: fix me, uncomment clouds         
         IF (ICLD .EQ. 1) CALL CLDPROP(ICLDATM, IAER, NSTR, IOUT,
     &    ISTART, IEND, ICLD)

         CALL SETCOEF(IAER, NSTR, IOUT, ISTART, IEND, ICLD,
     &                  idelm, isccos)

C ***    Call the radiative transfer routine.
         CALL RTRDIS_dg(IAER, NSTR, IOUT, ISTART, IEND, ICLD,
     &                   idelm, isccos)

         IF (IOUT .LT. 0) GO TO 4000

C ***    Process output for this atmosphere.
C         OPEN (IWR,FILE=outputFileName,FORM='FORMATTED')
 ...
 1000    CONTINUE
         ...
         WRITE(*,9899)WAVENUM1(ISTART),WAVENUM2(IEND)
         WRITE(*,9900)
         WRITE(*,9901)
C
...
C ***    Output module version numbers
         ...
         WRITE(*,9910) HNAMRTM, HVRRTM, HNAMATM, HVRATM,
...
C************************  SUBROUTINE READCLD  *****************************C
      ...
      SUBROUTINE READCLD(IAER, NSTR, IOUT, ISTART, IEND, ICLD,
     &                   idelm, isccos)
...
c      OPEN(IRDCLD,FILE=inputFN,FORM='FORMATTED')
...
C************************  SUBROUTINE READAER  *****************************C
      ...
      SUBROUTINE READAER(IAER, NSTR, IOUT, ISTART, IEND, ICLD,
     &                   idelm, isccos)
...
c      OPEN(IRDAER,FILE=trim(adjustl(inputFN)),FORM='FORMATTED')
