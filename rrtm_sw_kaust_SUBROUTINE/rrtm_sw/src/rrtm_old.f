...
C     revision:  $Revision: 26186 $
C     created:   $Date: 2014-12-02 14:09:12 -0500 (Tue, 02 Dec 2014) $
C
C  --------------------------------------------------------------------------
C |                                                                          |
...
c     Use unique ID from input to read and write correct files
c     This allows running several instances at the same time
...
c     read in current postfix to use it with filenames
      if (command_argument_count().gt.0) then
        call get_command_argument(1,inputArg)
      endif

c     construct correct new file names for input and output
...
C ***    Input atmospheric profile from INPUT_RRTM.
         CALL READPROF(inputFileName,inputAerFileName,
...
C        by interpolating data from stored reference atmospheres. 
         ICLDATM = 0
         IF (ICLD .EQ. 1) CALL CLDPROP(ICLDATM, outputCldFileName)
...
C************************  SUBROUTINE READPROF  *****************************C
      ...
      SUBROUTINE READPROF(inputFN, inputAerFN, inputCldFN, outputTapeFN)
...
c     main input file name
      character(len=210) :: inputFN
c     aerosols and clouds input file names
...
C  Initialize molecular amount and cross section arrays to zero here.
 ...
 1200 CONTINUE
      ...
      IXMAX = MAXINPX
      IRD = 9
c     use INPUT_RRTM + index as input filename
      OPEN (IRD,FILE=inputFN,FORM='FORMATTED')
...
C     If clouds are present, read in appropriate input file, IN_CLD_RRTM.
      IF (ICLD .EQ. 1) CALL READCLD(inputCldFN)
...
C               aerosol properties.
     ...
     &               10590.,14420.,18970.,25015.,30390.,43507.,1412./
      ...
      eps = 1.e-10
      IRDAER = 12
c     Use (IN_AER_RRTM + args) instead of default name
      OPEN(IRDAER,FILE=trim(adjustl(inputFN)),FORM='FORMATTED')
